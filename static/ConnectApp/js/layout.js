...
function delete_post(id) {
    ...
    setTimeout(() => {
        ...
        fetch('/posts/post/'+parseInt(id)+'/delete', {
            method: 'PUT'
        });
    },200);
}
...
function edit_post_submit(post_id) {
    ...
    fetch('/posts/post/'+parseInt(post_id)+'/edit', {
        method:'POST',
        body: formdata
    })
...
function like_post(element) {
    ...
    fetch('/posts/post/'+parseInt(id)+'/like', {
        method: 'PUT'
    })
    .then(() => {
...
function unlike_post(element) {
    let id = element.dataset.post_id;
    fetch('/posts/post/'+parseInt(id)+'/unlike', {
        method: 'PUT'
    })
    .then(() => {
        let count = element.querySelector('.likes_count');
...
function save_post(element) {
    ...
    let id = element.dataset.post_id;
    fetch('/posts/post/'+parseInt(id)+'/save', {
        method: 'PUT'
    })
    .then(() => {
        element.querySelector('.svg-span').innerHTML = `
...
function unsave_post(element) {
    let id = element.dataset.post_id;
    fetch('/posts/post/'+parseInt(id)+'/unsave', {
        method: 'PUT'
    })
    .then(() => {
        element.querySelector('.svg-span').innerHTML = `
...
function show_comment(element) {
    ...
    fetch('/posts/post/'+parseInt(post_id)+'/comments')
    .then(response => response.json())
    .then(comments => {
...
function write_comment(element) {
    ...
    fetch('/posts/post/'+parseInt(post_id)+'/write_comment',{
        method: 'POST',
        body: JSON.stringify({
